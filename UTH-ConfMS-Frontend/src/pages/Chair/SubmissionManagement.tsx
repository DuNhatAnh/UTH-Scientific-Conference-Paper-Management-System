...
export const SubmissionManagement: React.FC<SubmissionManagementProps> = ({ onNavigate }) => {
    ...
    const loadSubmissions = async () => {
        try {
            const res = await reviewApi.getSubmissionsForDecision();
            ...
            <div className="w-full max-w-[1200px] flex flex-col gap-6">
                ...
                <div className="bg-white dark:bg-card-dark rounded-xl border border-border-light shadow-sm overflow-hidden">
                    ...
                    {loading ? (
                        ...
                        <div className="p-8 text-center text-gray-500">Chưa có bài nộp nào đang chờ duyệt.</div>
                    ) : (
                                ...
                                {submissions.map((sub) => (
                                    <tr key={sub.submissionId} className="hover:bg-gray-50 transition-colors">
                                        ...
                                        <td className="p-3 text-center">
                                            <span className={`px-2 py-1 rounded-full text-xs font-bold ${sub.completedReviews === sub.totalReviews ? 'bg-green-100 text-green-700' : 'bg-yellow-100 text-yellow-700'}`}>
